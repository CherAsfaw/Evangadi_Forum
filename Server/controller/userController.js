...
async function register(req, res) {
	...
	try {
		const [
			user
		] = await dbConnection.query('SELECT user_name, user_id FROM userTable WHERE user_name=? or email=?', [
			user_name,
			email
		]);
...
async function login(req, res) {
	...
	try {
		const [ user ] = await dbConnection.query('SELECT user_name, user_id, password FROM userTable WHERE email=?', [
			email
		]);
